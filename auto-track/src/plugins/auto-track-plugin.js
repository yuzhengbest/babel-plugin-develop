 ...
 * 3. 对需要埋点的节点插入埋点逻辑，此时，需要兼容两种情况，一种是BlockStatement，直接插入 埋点逻辑；另一种，则需要将源码进行替换
 */
          ...
          path.traverse({ // 遍历当前的程序节点
            ImportDeclaration(curPath){ // 当遍历到当前的import 声明语句
              ...
              if (requirePath === options.trackerPath) {
                ...
                state.trackerImportId = specifiersPath.get('local').toString()
                // if (specifiersPath.isImportSpecifier()) {
                //   state.trackerImportId = specifiersPath.toString()
                // } else if (specifiersPath.isImportDefaultSpecifier() || specifiersPath.isImportNamespaceSpecifier()) {
                //   state.trackerImportId = specifiersPath.get('local').toString()
                // }
